...
export enum NotificationType {INFO,SUCCESS,WARNING,ERROR}
...
export function reducer(state = initialState, action:Action):StateInterface {
  ...
  switch (action.type) {
    ...
    default:
      if (succesMsg) {
        const notification = getNotificationWithKey({type:NotificationType.SUCCESS, message: "Account verified", timeout: 10000})
        return state.set(notification.notificationKey, notification)
      }
      return state;
  }
