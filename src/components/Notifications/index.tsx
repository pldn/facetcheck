...
export default class Notifications extends React.PureComponent<Notifications.Props,any> {
  ...
  **/
  willLeave():any {
    ...
    return {maxHeight: spring(0),paddingTop: spring(0), paddingBottom: spring(0), opacity: spring(0),marginTop: spring(0), fontSize: spring(0)};
  }
  willEnter() {
      return {
          maxHeight: 0,
        ...
        {(interpolatedStyles:any) =>
          <div>
            {interpolatedStyles.map((config:any) => {
              return <Notification closeHandler={closeHandler} key={config.key} {...config.data} style={config.style}/>
            })}
          </div>
        }
        </TransitionMotion>
