      ...
      <div className={getClassName(classNames, this.props.className)} style={style}>
        ...
        <div className={styles.notificationBody}>
          ...
          <div className={styles.message}>
            {title && <div className={styles.notificationHeader} style={{fontSize: +style.fontSize * 1.2}}>{title}</div>}
            <div>{message}</div>
            {devMessage && <div className={styles.devMessage}><div>You're special, you get a more detailed message:</div> {devMessage}</div>}
          </div>
          <button onClick={() => closeHandler(notificationKey)} type="button" className={styles.closeBtn} style={{fontSize: +style.fontSize * 1.5}} aria-label="Close"><span aria-hidden="true">&times;</span></button>
        </div>
      </div>
    );
  }
